...
- (void)quadCurveMenuItemTouchesEnd:(QuadCurveMenuItem *)item
{
    ...
    [self performBlock:^{ item.alpha = 0.0f; }
            afterDelay:0.25f];
    ...
    for (int i = 0; i < [_menusArray count]; i ++)
    {
        ...
        [self performBlock:^{ otherItem.alpha = 0.0f; }
                afterDelay:0.25f];
    }
    _expanding = NO;
...
- (void)_setMenu {
    ...
    for (int i = 0; i < count; i ++)
    {
        ...
        CGPoint p = RotateCGPointAroundCenter(endPoint, startPoint, rotateAngle);
        item.endPoint = CGPointMake( floorf(p.x), floorf(p.y) );
